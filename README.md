...
1. เพิ่ม Node "Code"
2. ตั้งชื่อ: `Calculate Grade`
3. เขียน JavaScript ตามตัวอย่างด้านล่าง
...
### Test Case 1: Basic (Grade A)
```json
{"student_id": "001", "name": "John Doe", "scores": [80, 80, 80]}
```
Expected: total=240, average=80.0, grade="A", status="passed"

### Test Case 2: Grade B
```json
{"student_id": "002", "name": "Jane Smith", "scores": [70, 75, 78]}
```
Expected: total=223, average=74.3, grade="B", status="passed"

### Test Case 3: Failed
```json
{"student_id": "003", "name": "Bob Wilson", "scores": [30, 40, 45]}
```
...
- ต้องกด **"Test workflow"** ก่อนทุกครั้งที่จะทดสอบด้วย curl
- Webhook path ต้องเป็น `lab01` (ไม่ใช่ `/lab01` หรือ `webhook-test/lab01`)
- Response Mode ต้องเป็น **"Last Node"** เพื่อให้ส่ง JSON กลับได้
